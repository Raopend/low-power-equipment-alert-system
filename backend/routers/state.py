...
def get_date_format(interval: TimeInterval) -> str:
    if interval == TimeInterval.minute:
        ...
        return "%Y-%m-%d %H:30:00" if "30" <= datetime.strftime(datetime.now(), "%M") < "60" else "%Y-%m-%d %H:00:00"
...
async def get_equipment_I_trend(params: TrendQueryParams, db: Session = Depends(get_db_session)):
    ...
    query = db.query(
        func.date_format(EquipmentState.collection_time, time_format).label("time_interval"),
...
async def get_equipment_U_trend(params: TrendQueryParams, db: Session = Depends(get_db_session)):
    # params start_time 减去 2 年
    params.start_time = params.start_time - relativedelta(years=2)
    # params end_time 减去 2 年
    params.end_time = params.end_time - relativedelta(years=2)
