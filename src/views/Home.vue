...
const fillIChartData = async () => {
  ...
  try {
    const response = await axiosInstance.post(
      "http://localhost:8000/query/equipment_I_trend",
      params
    );
    chartIData.value = {
      ...
      datasets: [
        datasetObject(
          "primary",
          "Ia",
          response.data.map((item) => item.Ia)
        ),
        datasetObject(
          "info",
          "Ib",
        ...
        datasetObject(
          "danger",
          "Ic",
          response.data.map((item) => item.Ic)
        ),
      ],
    };
  } catch (error) {
...
const fillUChartData = async () => {
  ...
  try {
    const response = await axiosInstance.post(
      "http://localhost:8000/query/equipment_U_trend",
      params
    );
    chartUData.value = {
      labels: response.data.map((item) => item.collection_time),
      datasets: [
        datasetObject(
          "primary",
          "Ua",
          response.data.map((item) => item.Ua)
        ),
        datasetObject(
          "info",
          "Ub",
          response.data.map((item) => item.Ub)
        ),
        datasetObject(
          "danger",
          "Uc",
          response.data.map((item) => item.Uc)
        ),
      ],
    };
  } catch (error) {
